...
import type { TaskTrendData, TimeRange } from '@/types/analytics'
import { createLineChartOption } from '@/utils/chart'

interface Props {
  data: TaskTrendData | null
  period: TimeRange
  height?: number
}
...
const initChart = () => {
  ...
  const series = [
    ...
    {
      name: '进行中',
      data: props.data.data.map(item => item.inProgress),
      smooth: true,
      areaStyle: {
        opacity: 0.3
      }
    ...
    {
      name: '已完成',
      data: props.data.data.map(item => item.completed),
      smooth: true,
      areaStyle: {
        opacity: 0.3
      }
