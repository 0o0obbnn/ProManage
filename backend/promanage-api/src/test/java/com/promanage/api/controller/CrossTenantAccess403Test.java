 ...
 *
 * <p>测试场景：用户尝试访问不属于其组织的资源，应返回403 Forbidden
 *
...
class CrossTenantAccess403Test {
  ...
  void getOrganization_crossTenant_shouldReturn403() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/{id}", 2L) // 访问组织B
                .with(
                    user("mockUser")
  ...
  void getOrganizationMembers_crossTenant_shouldReturn403() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/{id}/members", 2L) // 获取组织B的成员
                .param("page", "1")
                .param("pageSize", "20")
                .with(
                    user("mockUser")
  ...
  void updateOrganizationSettings_crossTenant_shouldReturn403() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/{id}/settings", 2L) // 更新组织B设置
  ...
  void deactivateOrganization_crossTenant_shouldReturn403() throws Exception {
    ...
    doThrow(new RuntimeException("无权操作"))
        ...
        .deactivateOrganization(eq(2L), eq(100L));

    mockMvc
        .perform(
            put("/api/v1/organizations/{id}/deactivate", 2L) // 停用组织B
                .with(
                    user("mockUser")
                        .authorities(new SimpleGrantedAuthority("ORGANIZATION_UPDATE"))
                        .password("password"))) // 用户属于组织A
  ...
  void activateOrganization_crossTenant_shouldReturn403() throws Exception {
    ...
    doThrow(new RuntimeException("无权操作"))
        .when(organizationService)
        .activateOrganization(eq(2L), eq(100L));

    mockMvc
        .perform(
            put("/api/v1/organizations/{id}/activate", 2L) // 激活组织B
                .with(
                    user("mockUser")
                        .authorities(new SimpleGrantedAuthority("ORGANIZATION_UPDATE"))
                        .password("password"))) // 用户属于组织A
  ...
  void updateSubscriptionPlan_crossTenant_shouldReturn403() throws Exception {
    ...
    doThrow(new RuntimeException("无权操作"))
        .when(organizationService)
        .updateSubscriptionPlan(eq(2L), anyString(), any(), eq(100L));

    mockMvc
        .perform(
            put("/api/v1/organizations/{id}/subscription", 2L) // 更新组织B订阅
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void deleteOrganization_crossTenant_shouldReturn403() throws Exception {
    ...
    doThrow(new RuntimeException("无权操作"))
        .when(organizationService)
        .deleteOrganization(eq(2L), eq(100L));

    mockMvc
        .perform(
            put("/api/v1/organizations/{id}/delete", 2L) // 删除组织B
                .with(
                    user("mockUser")
  ...
  void serviceLayer_crossTenant_shouldThrowException() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/{id}/members", 2L)
                .param("page", "1")
                .param("pageSize", "20")
                .with(
                    user("mockUser")
