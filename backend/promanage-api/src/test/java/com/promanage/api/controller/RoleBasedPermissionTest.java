...
class RoleBasedPermissionTest {
    ...
    void admin_canUpdateOrganizationSettings() throws Exception {
        ...
        when(organizationService.updateOrganizationSettings(eq(1L), any(OrganizationSettingsDTO.class), eq(100L)))
                .thenReturn(settings);

        mockMvc.perform(put("/api/v1/organizations/{id}/settings", 1L)
    ...
    void admin_canActivateOrganization() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/organizations/{id}/activate", 1L)
                        .with(user("mockUser").authorities(new SimpleGrantedAuthority("ORGANIZATION_UPDATE")).password("password")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void admin_canDeactivateOrganization() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/organizations/{id}/deactivate", 1L)
                        .with(user("mockUser").authorities(new SimpleGrantedAuthority("ORGANIZATION_UPDATE")).password("password")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void admin_canUpdateSubscriptionPlan() throws Exception {
        ...
        doNothing().when(organizationService).updateSubscriptionPlan(eq(1L), eq("ENTERPRISE"), any(LocalDateTime.class), eq(100L));


        mockMvc.perform(put("/api/v1/organizations/{id}/subscription", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void member_cannotUpdateOrganizationSettings_shouldReturn403() throws Exception {
        ...
        when(organizationService.updateOrganizationSettings(eq(1L), any(OrganizationSettingsDTO.class), eq(200L)))
    ...
    void member_cannotActivateOrganization_shouldReturn403() throws Exception {
        doThrow(new RuntimeException("无权操作")).when(organizationService).activateOrganization(eq(1L), eq(200L));
    ...
    void member_cannotDeactivateOrganization_shouldReturn403() throws Exception {
        doThrow(new RuntimeException("无权操作")).when(organizationService).deactivateOrganization(eq(1L), eq(200L));
    ...
    void member_cannotUpdateSubscriptionPlan_shouldReturn403() throws Exception {
        ...
        doThrow(new RuntimeException("无权操作")).when(organizationService).updateSubscriptionPlan(eq(1L), eq("ENTERPRISE"), any(LocalDateTime.class), eq(200L));
    ...
    void bothAdminAndMember_canListOrganizations() throws Exception {
        ...
        // 管理员查看
        when(organizationService.listOrganizations(eq(100L), eq(1), eq(20), nullable(String.class), nullable(Boolean.class)))
        ...
        // 成员查看
        when(organizationService.listOrganizations(eq(200L), eq(1), eq(20), nullable(String.class), nullable(Boolean.class)))
                .thenReturn(pageResult);
    ...
    void bothAdminAndMember_canGetOrganizationDetails() throws Exception {
        // 管理员查看
        when(organizationService.getOrganizationById(eq(1L), eq(100L)))
    ...
    void bothAdminAndMember_canListOrganizationMembers() throws Exception {
        OrganizationMemberDTO memberDTO = OrganizationMemberDTO.builder()
                .id(300L)
                .username("testmember")
        ...
        PageResult<OrganizationMemberDTO> pageResult = PageResult.of(Arrays.asList(memberDTO), 1L, 1, 20);
    ...
    void bothAdminAndMember_canGetOrganizationSettings() throws Exception {
        ...
        // 管理员查看
        when(organizationService.getOrganizationSettings(eq(1L), eq(100L)))
                .thenReturn(settings);

        mockMvc.perform(get("/api/v1/organizations/{id}/settings", 1L)
                        .with(user("mockUser").authorities(new SimpleGrantedAuthority("ORGANIZATION_VIEW")).password("password")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));

        // 成员查看
        when(organizationService.getOrganizationSettings(eq(1L), eq(200L)))
                .thenReturn(settings);
    ...
    void admin_withoutUpdatePermission_cannotPerformAdminActions() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/organizations/{id}/settings", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(settings))
                        .with(user("mockUser").authorities(new SimpleGrantedAuthority("ORGANIZATION_VIEW")).password("password"))) // 只有查看权限
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.code").value(403));
    }
