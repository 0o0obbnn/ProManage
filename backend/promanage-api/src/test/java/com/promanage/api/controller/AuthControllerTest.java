 ...
 *
 * <p>基于 ProManage API 规范全面测试认证相关API接口
 *
...
class AuthControllerTest {
    ...
    mockMvc
        .perform(
            post("/api/v1/auth/refresh")
                ...
                .content("valid_refresh_token_here"))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(200))
        .andExpect(jsonPath("$.data.token").exists())
  ...
  @Test
  @DisplayName("刷新访问令牌 - 刷新令牌已加入黑名单")
  void shouldReturnError_whenRefreshTokenIsBlacklisted() throws Exception {
    // when & then
    mockMvc
        .perform(
            post("/api/v1/auth/refresh")
                .contentType(MediaType.TEXT_PLAIN)
                .content("blacklisted_refresh_token"))
        .andDo(print())
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.code").value(401))
