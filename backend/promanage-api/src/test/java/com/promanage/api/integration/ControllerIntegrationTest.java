...
class ControllerIntegrationTest {
  ...
  void testCompleteApiFlow() throws Exception {
    ...
    // 4. 获取权限列表
    mockMvc
        .perform(get("/api/v1/permissions").header("Authorization", "Bearer " + authToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(200))
    ...
    // 5. 获取当前用户信息
    mockMvc
        .perform(get("/api/v1/auth/me").header("Authorization", "Bearer " + authToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(200))
        .andExpect(jsonPath("$.data.username").value("testuser"));
    ...
    // 6. 用户登出
    mockMvc
        .perform(post("/api/v1/auth/logout").header("Authorization", "Bearer " + authToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(200));
  }
  ...
  void testTestCaseManagementApiFlow() throws Exception {
    ...
    Map<String, Object> createDataMap =
        objectMapper.convertValue(
            createResultData.getData(),
            new com.fasterxml.jackson.core.type.TypeReference<Map<String, Object>>() {});
    ...
    mockMvc
        .perform(
            get("/api/v1/test-cases/" + testCaseId).header("Authorization", "Bearer " + authToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(200))
  ...
  void testUserManagementApiFlow() throws Exception {
    ...
    // 3. 获取用户详情
    mockMvc
        .perform(get("/api/v1/users/" + userId).header("Authorization", "Bearer " + authToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.code").value(200))
  ...
  void testPermissionValidation() throws Exception {
    ...
    // 3. 尝试访问无权限的API（应该失败）
    mockMvc
        .perform(delete("/api/v1/users/1").header("Authorization", "Bearer " + authToken))
        .andExpect(status().isForbidden());
