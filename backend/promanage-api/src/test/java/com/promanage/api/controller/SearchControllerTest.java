...
class SearchControllerTest {
  ...
  void globalSearch_withValidParameters_shouldReturnResults() throws Exception {
    // Given
    PageResult<ISearchService.SearchResult> searchResult =
        PageResult.of(
            Arrays.asList(
  ...
  void searchDocuments_withValidKeyword_shouldReturnDocuments() throws Exception {
    // Given
    PageResult<ISearchService.SearchResult> documentResult =
        PageResult.of(
            Arrays.asList(createTestSearchResult("document", 1L, "Test Document", "Test content")),
            1L,
            1,
  ...
  void searchProjects_withValidKeyword_shouldReturnProjects() throws Exception {
    // Given
    PageResult<ISearchService.SearchResult> projectResult =
        PageResult.of(
            Arrays.asList(
                createTestSearchResult("project", 1L, "Test Project", "Test description")),
            1L,
            1,
            20);
    when(searchService.searchProjects(anyString(), anyInt(), anyInt())).thenReturn(projectResult);
  ...
  void searchTasks_withValidKeyword_shouldReturnTasks() throws Exception {
    // Given
    PageResult<ISearchService.SearchResult> taskResult =
        PageResult.of(
            Arrays.asList(createTestSearchResult("task", 1L, "Test Task", "Test task description")),
            1L,
            1,
            20);
    when(searchService.searchTasks(anyString(), any(), anyInt(), anyInt())).thenReturn(taskResult);
  ...
  void searchTasks_withProjectFilter_shouldReturnFilteredTasks() throws Exception {
    ...
    PageResult<ISearchService.SearchResult> taskResult =
        PageResult.of(
            ...
            1,
            20);
    when(searchService.searchTasks(anyString(), eq(1L), anyInt(), anyInt())).thenReturn(taskResult);
  ...
  void globalSearch_withEmptyResults_shouldReturnEmptyList() throws Exception {
    // Given
    PageResult<ISearchService.SearchResult> searchResult =
        PageResult.of(Collections.emptyList(), 0L, 1, 20);
    when(searchService.globalSearch(anyString(), anyString(), any(), anyInt(), anyInt()))
        .thenReturn(searchResult);
  ...
  private ISearchService.SearchResult createTestSearchResult(
      String type, Long id, String title, String content) {
