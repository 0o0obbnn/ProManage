 ...
 * <p>
 * 基于Testcontainers/PostgreSQL验证软删除过滤功能
 * </p>
 *
...
class SoftDeleteFilterIntegrationTest extends PostgreSQLContainerBaseTest {
    ...
    void defaultQuery_shouldFilterDeletedRecords() throws Exception {
        ...
        when(organizationService.listOrganizations(eq(1L), eq(1), eq(20), nullable(String.class), nullable(Boolean.class)))
    ...
    void includeDeletedQuery_shouldReturnAllRecords() throws Exception {
        ...
        when(organizationService.listOrganizations(eq(1L), eq(1), eq(30), nullable(String.class), eq(true)))
                .thenReturn(pageResult);
    ...
    void onlyDeletedQuery_shouldReturnOnlyDeletedRecords() throws Exception {
        ...
        when(organizationService.listOrganizations(eq(1L), eq(1), eq(20), nullable(String.class), eq(false)))
                .thenReturn(pageResult);
    ...
    void paginatedQuery_shouldFilterDeletedRecords() throws Exception {
        ...
        when(organizationService.listOrganizations(eq(1L), eq(1), eq(10), nullable(String.class), nullable(Boolean.class)))
                .thenReturn(pageResult);
    ...
    void keywordSearch_shouldFilterDeletedRecords() throws Exception {
        ...
        when(organizationService.listOrganizations(eq(1L), eq(1), eq(20), eq("活跃"), nullable(Boolean.class)))
                .thenReturn(pageResult);
    ...
    void statusFilter_shouldCombineWithSoftDeleteFilter() throws Exception {
        ...
        when(organizationService.listOrganizations(eq(1L), eq(1), eq(20), nullable(String.class), eq(true)))
                .thenReturn(pageResult);
    ...
    void bulkOperations_shouldHandleSoftDeletedRecords() throws Exception {
        ...
        when(organizationService.listOrganizations(eq(1L), eq(1), eq(50), nullable(String.class), nullable(Boolean.class)))
                .thenReturn(pageResult);
    ...
    void softDeletedRecord_detailsQuery_shouldReturn404() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/organizations/{id}", 21L) // 已删除组织的ID
                        .with(user("mockUser").authorities(new SimpleGrantedAuthority("ORGANIZATION_VIEW")).password("password")))
    ...
    void softDeletedRecord_updateOperation_shouldReturn404() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/organizations/{id}", 21L) // 已删除组织的ID
    ...
    void softDeletedRecord_deleteOperation_shouldReturn404() throws Exception {
        ...
        doThrow(new RuntimeException("组织不存在"))
                .when(organizationService).deleteOrganization(eq(21L), eq(1L));

        mockMvc.perform(delete("/api/v1/organizations/{id}", 21L) // 已删除组织的ID
                        .with(user("mockUser").authorities(new SimpleGrantedAuthority("ORGANIZATION_DELETE")).password("password")))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value(404))
                .andExpect(jsonPath("$.message").value("组织不存在"));
    }
