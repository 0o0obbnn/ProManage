 ...
 *
 * <p>基于Testcontainers/PostgreSQL验证订阅过期过滤功能
 *
...
class SubscriptionExpiryIntegrationTest extends PostgreSQLContainerBaseTest {
  ...
  void expiredSubscription_operations_shouldBeRestricted() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/organizations/{id}", 16L) // 已过期订阅的组织
                .with(
                    user("mockUser")
                        .authorities(new SimpleGrantedAuthority("ORGANIZATION_VIEW"))
                        .password("password")))
  ...
  void expiredSubscription_updateOperation_shouldReturn403() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/{id}", 16L) // 已过期订阅的组织
  ...
  void expiredSubscription_settingsUpdate_shouldReturn403() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/organizations/{id}/settings", 16L) // 已过期订阅的组织
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void renewExpiredSubscription_shouldSucceed() throws Exception {
    ...
    doNothing()
        ...
        .updateSubscriptionPlan(eq(16L), eq("PRO"), any(LocalDateTime.class), eq(1L));

    mockMvc
        .perform(
            put("/api/v1/organizations/{id}/subscription", 16L) // 续费已过期订阅的组织
                .contentType(MediaType.APPLICATION_JSON)
                .content(renewalJson)
                .with(
                    user("mockUser")
