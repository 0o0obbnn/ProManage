...
public class AuthController {
    ...
    public Result<UserResponse> register(@Valid @RequestBody RegisterRequest registerRequest) {
        ...
        if (!registerRequest.getPassword().equals(registerRequest.getConfirmPassword())) {
            log.warn("两次密码不一致, username={}", registerRequest.getUsername());
            return Result.error(ResultCode.BAD_REQUEST.getCode(), "两次密码不一致");
        }
    ...
    public Result<LoginResponse> refresh(@RequestBody String refreshToken) {
        ...
        // 构建权限列表
        String authoritiesStr = roles.stream()
    ...
    public Result<UserResponse> getCurrentUser(Authentication authentication, HttpServletResponse responseWrapper) {
        // 设置禁止缓存的HTTP头，避免304状态码问题
        responseWrapper.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
        responseWrapper.setHeader("Pragma", "no-cache");
        responseWrapper.setHeader("Expires", "0");
    ...
    public Result<Void> resetPassword(@Valid @RequestBody ResetPasswordRequest request) {
        ...
        if (!request.getNewPassword().equals(request.getConfirmPassword())) {
            log.warn("两次密码不一致, email={}", request.getEmail());
            return Result.error(ResultCode.BAD_REQUEST.getCode(), "两次密码不一致");
        }
    ...
    public Result<PasswordStrengthResponse> checkPasswordStrength(
        ...
        PasswordStrengthResponse response = passwordService.checkPasswordStrength(request.getPassword());
    ...
    public Result<List<String>> getUserPermissions(Authentication authentication, HttpServletResponse responseWrapper) {
        // 设置禁止缓存的HTTP头，避免304状态码问题
        responseWrapper.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
        responseWrapper.setHeader("Pragma", "no-cache");
        responseWrapper.setHeader("Expires", "0");
        ...
        // 获取用户权限
        List<com.promanage.service.entity.Permission> permissions = userService.getUserPermissions(user.getId());
        List<String> permissionCodes = permissions.stream()
                .map(com.promanage.service.entity.Permission::getPermissionCode)
                .collect(Collectors.toList());
