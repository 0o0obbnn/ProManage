...
public class SearchController {
  ...
  public Result<PageResult<SearchResponse>> globalSearch(
    ...
    log.info(
        "全局搜索请求, q={}, type={}, projectId={}, page={}, size={}", q, type, projectId, page, size);
    ...
    List<SearchResponse> responseList =
        searchResults.getList().stream().map(this::convertToResponse).collect(Collectors.toList());

    PageResult<SearchResponse> response =
        PageResult.of(
            responseList,
  ...
  public Result<PageResult<Document>> searchDocuments(
      ...
      @Parameter(description = "页码") @RequestParam(defaultValue = "1") Integer page,
      @Parameter(description = "每页大小") @RequestParam(defaultValue = "20") Integer size) {
    log.info("搜索文档请求, q={}, projectId={}, page={}, size={}", q, projectId, page, size);

    PageResult<ISearchService.SearchResult> searchResults =
        searchService.searchDocuments(q, projectId, page, size);
  ...
  public Result<PageResult<Task>> searchTasks(
      ...
      @Parameter(description = "页码") @RequestParam(defaultValue = "1") Integer page,
      @Parameter(description = "每页大小") @RequestParam(defaultValue = "20") Integer size) {
    log.info("搜索任务请求, q={}, projectId={}, page={}, size={}", q, projectId, page, size);

    PageResult<ISearchService.SearchResult> searchResults =
        searchService.searchTasks(q, projectId, page, size);
   ...
   * @return 文档分页结果
   */
  private PageResult<Document> convertSearchResultToDocumentPage(
      PageResult<ISearchService.SearchResult> searchResults) {
    // 这里需要实现从SearchResult到Document的转换逻辑
    // 由于SearchResult只包含基本信息，可能需要调用其他服务获取完整Document对象
    // 暂时返回空结果，实际实现需要根据业务逻辑补充
    return PageResult.of(
        new java.util.ArrayList<>(), 0L, searchResults.getPage(), searchResults.getPageSize());
  }
   ...
   * @return 项目分页结果
   */
  private PageResult<Project> convertSearchResultToProjectPage(
      PageResult<ISearchService.SearchResult> searchResults) {
   ...
   * @return 任务分页结果
   */
  private PageResult<Task> convertSearchResultToTaskPage(
      PageResult<ISearchService.SearchResult> searchResults) {
