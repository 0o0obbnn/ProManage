...
import com.promanage.api.constant.ApiConstants;
 ...
 * 用户管理控制器
 *
 * <p>提供用户管理的API接口，包括用户的CRUD操作、角色管理等功能
 *
...
public class UserController {
  ...
  public Result<PageResult<UserResponse>> listUsers(
      @Parameter(description = "页码，从1开始", example = "1") @RequestParam(defaultValue = "1")
          Integer page,
      @Parameter(description = "每页大小", example = "20") @RequestParam(defaultValue = "20")
          Integer pageSize,
      @Parameter(description = "搜索关键词", example = "admin") @RequestParam(required = false)
          String keyword) {
    ...
    List<UserResponse> userResponses =
        pageResult.getList().stream().map(this::convertToUserResponse).collect(Collectors.toList());

    PageResult<UserResponse> response =
        PageResult.<UserResponse>builder()
            .list(userResponses)
  ...
  public Result<UserResponse> getUserById(
      @Parameter(description = ApiConstants.FIELD_USER_ID, example = "1") @PathVariable Long id) {
  ...
  public Result<Void> addRole(
      @Parameter(description = ApiConstants.FIELD_USER_ID, example = "1") @PathVariable Long id,
      @Parameter(description = ApiConstants.FIELD_ROLE_ID, example = "1") @PathVariable Long roleId) {
