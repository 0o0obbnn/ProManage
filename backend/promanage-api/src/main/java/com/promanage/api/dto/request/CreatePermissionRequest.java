...
public class CreatePermissionRequest {
    ...
    @Schema(description = "权限编码", example = "document:create", requiredMode = Schema.RequiredMode.REQUIRED)
    private String permissionCode;

    /**
     * 权限类型
     * <p>
     * 必填项，menu-菜单权限，button-按钮权限，api-API权限
     * </p>
     */
    @NotBlank(message = "权限类型不能为空")
    @Pattern(regexp = "^(menu|button|api)$", message = "权限类型必须是menu、button或api")
    @Schema(description = "权限类型 (menu/button/api)", example = "api", requiredMode = Schema.RequiredMode.REQUIRED)
    private String type;

    /**
     * API路径
     * <p>
     * 当type为api时必填，API请求路径
     * </p>
     */
