...
/**
 * 数据库迁移工具 用于在应用启动时执行必要的数据库Schema更新
 *
...
public class DatabaseMigrationUtil implements CommandLineRunner {
  ...
  private void addColumnIfNotExists(
      String columnName, String columnType, String defaultValue, String comment) {
    try {
      // 检查字段是否存在
      String checkSql =
      ...
      if (count == null || count == 0) {
        ...
        String alterSql =
            String.format(
                "ALTER TABLE tb_document ADD COLUMN %s %s DEFAULT %s",
        ...
        if (comment != null && !comment.isEmpty()) {
          String commentSql =
              String.format("COMMENT ON COLUMN tb_document.%s IS '%s'", columnName, comment);
