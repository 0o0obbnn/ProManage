...
public final class IpUtils {
   ...
   * 获取客户端真实IP地址
   *
   * <p>支持通过代理、负载均衡等方式获取真实IP 优先级：X-Forwarded-For > X-Real-IP > Proxy-Client-IP > WL-Proxy-Client-IP
   * > RemoteAddr
   *
  ...
  public static boolean isInternalIp(String ip) {
    ...
    if (parts.length == 4) {
      try {
        ...
        log.warn("Invalid IP format: {}", sanitizeLogInput(ip));
      }
    }
