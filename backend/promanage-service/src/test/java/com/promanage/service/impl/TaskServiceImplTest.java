...
class TaskServiceImplTest {
  ...
  @Mock private TaskDependencyMapper taskDependencyMapper;

  @Mock private ProjectMapper projectMapper;

  @Mock private UserMapper userMapper;

  @Mock private TaskActivityMapper taskActivityMapper;

  @Mock private TaskAttachmentMapper taskAttachmentMapper;

  @Mock private TaskCheckItemMapper taskCheckItemMapper;
  ...
  void testAddTaskActivity() {
    ...
    when(taskMapper.selectById(1L)).thenReturn(task);
    when(taskActivityMapper.insert(any(TaskActivity.class)))
        .thenAnswer(
            invocation -> {
  ...
  void testListTaskAttachments() {
    ...
    when(taskMapper.selectById(1L)).thenReturn(task);
    when(taskAttachmentMapper.selectList(any()))
  ...
  void testAddTaskAttachment() {
    ...
    when(taskMapper.selectById(1L)).thenReturn(task);
    when(taskAttachmentMapper.insert(any(TaskAttachment.class)))
        .thenAnswer(
            invocation -> {
  ...
  void testAddTaskCheckItem() {
    ...
    when(taskMapper.selectById(1L)).thenReturn(task);
    when(taskCheckItemMapper.insert(any(TaskCheckItem.class)))
        .thenAnswer(
            invocation -> {
