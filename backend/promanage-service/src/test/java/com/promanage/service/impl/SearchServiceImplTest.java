...
class SearchServiceImplTest {
    ...
    void testGlobalSearchSuccess() {
        ...
        // 模拟Mapper返回
        when(documentMapper.searchDocuments(eq("测试"), isNull(), eq(0), eq(10)))
                .thenReturn(documents);
        when(documentMapper.countSearchDocuments(eq("测试"), isNull()))
                .thenReturn(1L);
        
        when(projectMapper.searchProjects(eq("测试"), eq(0), eq(10)))
                .thenReturn(projects);
        when(projectMapper.countSearchProjects(eq("测试")))
                .thenReturn(1L);
        
        when(taskMapper.searchTasks(eq("测试"), isNull(), eq(0), eq(10)))
                .thenReturn(tasks);
        when(taskMapper.countSearchTasks(eq("测试"), isNull()))
                .thenReturn(1L);
    ...
    void testGetSearchSuggestionsSuccess() {
        ...
        // 模拟Mapper返回
        when(documentMapper.getDistinctTitlesByKeyword(eq("测试")))
                .thenReturn(docTitles);
        when(projectMapper.getDistinctNamesByKeyword(eq("测试")))
                .thenReturn(projNames);
        when(taskMapper.getDistinctTitlesByKeyword(eq("测试")))
                .thenReturn(taskTitles);
