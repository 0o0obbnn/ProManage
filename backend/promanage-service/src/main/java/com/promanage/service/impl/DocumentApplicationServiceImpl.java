...
public class DocumentApplicationServiceImpl implements IDocumentApplicationService {
  ...
  private final IDocumentVersionService
      documentVersionService; // Assuming this service exists or will be created
  ...
  private final CommentMapper
      commentMapper; // Temporarily keep mapper, should be replaced by ICommentService
  ...
  public DocumentDetailDTO getDocumentDetails(Long documentId) {
    ...
    List<DocumentVersion> versions =
        documentVersionService.listVersions(
            documentId,
            SecurityUtils.getCurrentUserId()
                .orElseThrow(() -> new BusinessException(ResultCode.UNAUTHORIZED, "用户未登录")));
    ...
    enrichWithStatistics(
        response, document, versions, favoriteCount, commentCount, weeklyViewCount);
  ...
  @Override
  @Transactional
  public DocumentDetailDTO createDocument(
      Long projectId, com.promanage.service.dto.request.CreateDocumentRequest request) {
    log.info(
        "Creating document via application service, projectId={}, title={}",
        projectId,
        request.getTitle());
  ...
  private void enrichWithVersions(DocumentDetailDTO response, List<DocumentVersion> versions) {
    ...
    List<DocumentVersionDTO> versionResponses =
        versions.stream()
            .map(
                version -> {
  ...
  private void enrichWithStatistics(
      DocumentDetailDTO response,
      Document document,
      List<DocumentVersion> versions,
      int favoriteCount,
      int commentCount,
      int weeklyViewCount) {
    DocumentStatisticsDTO statistics =
        DocumentStatisticsDTO.builder()
  ...
  private void enrichWithUserInfo(DocumentDetailDTO response) {
    ...
    if (creator != null) {
      response.setCreatorName(
          creator.getRealName() != null ? creator.getRealName() : creator.getUsername());
