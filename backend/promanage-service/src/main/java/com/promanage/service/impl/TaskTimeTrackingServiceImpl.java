...
public class TaskTimeTrackingServiceImpl
    extends ServiceImpl<TaskTimeTrackingMapper, TaskTimeTracking>
    implements ITaskTimeTrackingService {

  private final ITaskService taskService;

  @Override
  public PageResult<TaskTimeTrackingDTO> listTimeTracking(
      Long taskId,
      Long userId,
      Long projectId,
      Integer status,
      ...
      Integer size) {
    log.info(
        "查询任务时间追踪记录: taskId={}, userId={}, projectId={}, status={}, startDate={}, endDate={}, page={}, size={}",
        taskId,
        userId,
        projectId,
        status,
    ...
    List<TaskTimeTrackingDTO> dtoList =
        pageResult.getRecords().stream().map(this::convertToDTO).collect(Collectors.toList());
  ...
  @Override
  public List<TaskTimeTrackingDTO> getTimeTrackingByUserId(
  ...
  public Long createTimeTracking(TaskTimeTrackingDTO timeTrackingDTO) {
    log.info(
        "创建时间追踪记录: taskId={}, userId={}", timeTrackingDTO.getTaskId(), timeTrackingDTO.getUserId());
