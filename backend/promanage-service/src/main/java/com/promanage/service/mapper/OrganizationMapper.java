...
public interface OrganizationMapper extends BaseMapper<Organization> {
     ...
     */
    @Select("SELECT * FROM organizations WHERE created_by = #{createdBy} AND deleted_at IS NULL ORDER BY created_at DESC")
     ...
     * @return List<Organization> 组织列表
     */
    @Select("SELECT * FROM organizations WHERE is_active = #{isActive} AND deleted_at IS NULL ORDER BY created_at DESC")
     ...
     * @return List<Organization> 组织列表
     */
    @Select("SELECT * FROM organizations WHERE subscription_plan = #{subscriptionPlan} AND deleted_at IS NULL ORDER BY created_at DESC")
     ...
     * @return List<Organization> 组织列表
     */
    @Select("SELECT * FROM organizations WHERE name LIKE CONCAT('%', #{name}, '%') AND deleted_at IS NULL ORDER BY created_at DESC")
     ...
     * @return int 组织数量
     */
    @Select("SELECT COUNT(1) FROM organizations WHERE subscription_plan = #{subscriptionPlan} AND deleted_at IS NULL")
     ...
     * @return List<Organization> 组织列表
     */
    @Select("SELECT * FROM organizations WHERE subscription_expires_at <= CURRENT_TIMESTAMP AND deleted_at IS NULL")
