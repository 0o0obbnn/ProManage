...
public interface TaskDependencyMapper extends BaseMapper<TaskDependency> {
     ...
     * @return true表示存在依赖关系
     */
    @Select("SELECT COUNT(*) > 0 FROM tb_task_dependency WHERE prerequisite_task_id = #{prerequisiteTaskId} AND dependent_task_id = #{dependentTaskId}")
     ...
     * @return 删除的记录数
     */
    @Delete("DELETE FROM tb_task_dependency WHERE prerequisite_task_id = #{prerequisiteTaskId} AND dependent_task_id = #{dependentTaskId}")
     ...
     * @return 删除的记录数
     */
    @Delete("DELETE FROM tb_task_dependency WHERE prerequisite_task_id = #{taskId} OR dependent_task_id = #{taskId}")
