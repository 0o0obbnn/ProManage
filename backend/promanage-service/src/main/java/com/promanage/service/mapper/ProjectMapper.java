...
public interface ProjectMapper extends BaseMapper<Project> {
     ...
     */
    @Select("SELECT * FROM tb_project WHERE owner_id = #{ownerId} AND deleted = false ORDER BY create_time DESC")
     ...
     * @return List<Project> 项目列表
     */
    @Select("SELECT * FROM tb_project WHERE status = #{status} AND deleted = false ORDER BY create_time DESC")
     ...
     * @return List<Project> 项目列表
     */
    @Select("SELECT * FROM tb_project WHERE name LIKE CONCAT('%', #{name}, '%') AND deleted = false ORDER BY create_time DESC")
     ...
     * 根据成员ID查找项目列表
     * <p>
     * 通过项目成员关联表查询用户参与的所有项目
     * </p>
     *
     * @param memberId 成员ID
     * @return List<Project> 项目列表
     */
    @Select("SELECT p.* FROM tb_project p INNER JOIN tb_project_member pm ON p.id = pm.project_id WHERE pm.user_id = #{memberId} AND p.deleted = false AND pm.deleted = false ORDER BY p.create_time DESC")
