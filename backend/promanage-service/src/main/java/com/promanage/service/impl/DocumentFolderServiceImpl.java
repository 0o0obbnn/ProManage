...
public class DocumentFolderServiceImpl implements IDocumentFolderService {
  ...
  @Override
  public PageResult<DocumentFolder> listFolders(
      Integer page, Integer pageSize, Long projectId, Long parentId) {
    log.info(
        "分页查询文件夹列表, page={}, pageSize={}, projectId={}, parentId={}",
        page,
        pageSize,
        projectId,
        parentId);
   ...
   * @return 树形结构列表
   */
  private List<IDocumentFolderService.DocumentFolderTreeNode> buildFolderTree(
    ...
    // 转换为TreeNode列表
    List<IDocumentFolderService.DocumentFolderTreeNode> nodes =
        folders.stream().map(this::convertToTreeNode).collect(Collectors.toList());
  ...
  private IDocumentFolderService.DocumentFolderTreeNode convertToTreeNode(DocumentFolder folder) {
    IDocumentFolderService.DocumentFolderTreeNode node =
        new IDocumentFolderService.DocumentFolderTreeNode();
