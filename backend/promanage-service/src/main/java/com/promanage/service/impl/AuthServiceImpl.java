...
public class AuthServiceImpl implements IAuthService {
    ...
    public User authenticate(String username, String password) {
        ...
        log.info("DEBUG - username={}, dbPasswordIsNull={}, dbPasswordLength={}",
            username,
            dbPassword == null,
            dbPassword != null ? dbPassword.length() : 0
        );
        if (dbPassword != null && dbPassword.length() > 0) {
            log.info("DEBUG - dbPasswordPrefix={}", dbPassword.substring(0, Math.min(20, dbPassword.length())));
        }
    ...
    public User register(String username, String password, String email, String phone, String realName) {
        ...
        // 尝试获取用户名锁
        Boolean usernameLockAcquired = redisTemplate.opsForValue()
        ...
        try {
            // 尝试获取邮箱锁
            Boolean emailLockAcquired = redisTemplate.opsForValue()
            ...
            try {
                ...
                // 调用 userService.register 创建用户并分配默认角色
                // 捕获数据库层面的唯一约束违反异常（最后一道防线）
                Long userId;
                try {
                    userId = userService.register(user);
                    log.info("用户注册成功, username={}, userId={}", username, userId);
                } catch (DuplicateKeyException e) {
                    log.error("数据库唯一约束冲突，用户名或邮箱已存在, username={}, email={}", username, email, e);
                    // 判断是用户名还是邮箱冲突
                ...
                // 返回创建的用户（不包含密码）
                User registeredUser = userService.getById(userId);
