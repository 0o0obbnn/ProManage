 ...
 *
 * <p>Configures thread pool for asynchronous task execution. Enables @Async annotation for
 * background task processing.
 *
 * <p>Thread pool sizing guidelines: - Core pool size: Number of threads always alive - Max pool
 * size: Maximum number of threads - Queue capacity: Size of task queue before creating new threads
...
public class AsyncConfig implements AsyncConfigurer {
  ...
  /**
   * Core pool size - threads always alive Set based on CPU cores (typically 2 * cores for I/O bound
   * tasks)
   */
  private static final int CORE_POOL_SIZE = 10;

  /**
   * Maximum pool size - maximum number of threads Should be higher than core pool size to handle
   * load spikes
   */
  private static final int MAX_POOL_SIZE = 50;

  /**
   * Queue capacity - size of task queue Tasks exceeding this will trigger new thread creation (up
   * to max pool size)
   */
  private static final int QUEUE_CAPACITY = 200;

  /**
   * Keep alive time in seconds Idle threads exceeding core pool size will be terminated after this
   * time
   */
   ...
   *
   * <p>Creates a thread pool with the following characteristics: - Core threads: Always alive for
   * immediate task execution - Max threads: Created on demand when queue is full - Queue: Holds
   * tasks when all core threads are busy - Rejection policy: Caller runs (fallback to synchronous
   * execution)
   *
   ...
   *
   * <p>Handles exceptions that occur in @Async methods without return value. Logs the exception
   ...
   *
   * <p>Logs exception details including: - Method name where exception occurred - Exception type
   * and message - Stack trace for debugging
   */
  public static class CustomAsyncExceptionHandler implements AsyncUncaughtExceptionHandler {

    private static final org.slf4j.Logger log =
        org.slf4j.LoggerFactory.getLogger(CustomAsyncExceptionHandler.class);

    @Override
    public void handleUncaughtException(Throwable throwable, Method method, Object... params) {
      log.error(
          "Async task execution failed - Method: {}, Exception: {}",
          method.getName(),
    ...
    private void sendAsyncTaskAlert(Throwable throwable, Method method, Object... params) {
      // 这里可以集成邮件、短信、钉钉等告警渠道
      log.error(
          "异步任务告警: method={}, exception={}, message={}",
          method.getName(),
          throwable.getClass().getSimpleName(),
          throwable.getMessage());
    }
   ...
   *
   * <p>Example: Email sending executor with different configuration
   *
   ...
   *
   * <p>Higher capacity for file upload/download operations
   *
