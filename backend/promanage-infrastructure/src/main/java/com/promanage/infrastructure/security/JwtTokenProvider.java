 ...
 * JWT Token Provider
 *
 * <p>Handles JWT token generation, validation, and extraction of claims. Uses JJWT library with
 * HS512 signature algorithm for secure token management.
 *
...
public class JwtTokenProvider {
  ...
  /**
   * JWT secret key (minimum 512 bits for HS512) Should be configured in application.yml and kept
   * secure NO DEFAULT VALUE - must be configured in production
   */
  @Value("${jwt.secret}")
  ...
  /** JWT refresh token expiration time in milliseconds Default: 7 days (604800000 ms) */
  ...
  /**
   * Validate JWT configuration on application startup This ensures that weak or default secrets are
   * detected early
   */
  @PostConstruct
  public void validateConfiguration() {
    ...
    if (!JwtSecretGenerator.isStrongSecret(jwtSecret)) {
      ...
      log.error(
          "⚠️  Secret entropy: {} bits (minimum required: 256 bits)",
          String.format("%.2f", entropy));
      log.error(
          "⚠️  Secret length: {} characters (minimum required: 64 characters)", jwtSecret.length());
      log.error(
          "⚠️  SECURITY RISK: Weak JWT secrets can be cracked, leading to complete authentication bypass!");
      log.error("⚠️  Generate a strong secret immediately using one of these methods:");
      log.error(
          "⚠️    1. Run: java -cp your-app.jar com.promanage.infrastructure.security.JwtSecretGenerator");
    ...
    log.info(
        "✅ Secret entropy: {} bits",
        String.format("%.2f", JwtSecretGenerator.calculateEntropy(jwtSecret)));
    ...
    if (jwtRefreshExpirationMs <= jwtExpirationMs) {
      log.warn(
          "⚠️  Refresh token expiration ({} ms) should be longer than access token expiration ({} ms)",
    ...
    log.info("   - Secret length: {} characters", jwtSecret.length());
    log.info(
        "   - Access token expiration: {} ms ({} hours)",
        jwtExpirationMs,
        jwtExpirationMs / 3_600_000);
    log.info(
        "   - Refresh token expiration: {} ms ({} days)",
        jwtRefreshExpirationMs,
        jwtRefreshExpirationMs / 86_400_000);
  ...
  public boolean validateToken(String token) {
    try {
      ...
      log.error("JWT token validation failed with an unexpected error", e);
    }
    return false;
  }
  ...
  public boolean isTokenExpired(String token) {
    try {
      ...
      log.error("Failed to check token expiration, token is likely invalid", e);
      throw new RuntimeException("Failed to check token expiration, token is likely invalid", e);
    }
  }
