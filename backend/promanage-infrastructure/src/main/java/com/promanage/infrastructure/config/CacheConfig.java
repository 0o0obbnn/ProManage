 ...
 *
 * <p>Configures Spring Cache abstraction with custom key generation and error handling. Works with
 * RedisCacheManager configured in RedisConfig.
 *
...
public class CacheConfig implements CachingConfigurer {
   ...
   *
   * <p>Generates cache keys based on: - Class name - Method name - Method parameters
   *
   * <p>Example key format: com.promanage.service.UserService.getUserById[123]
   *
   ...
   * Cache error handler
   *
   * <p>Handles cache errors gracefully without breaking application flow. Logs errors but allows
   * the application to continue without cache.
   *
   ...
   * Cache resolver
   *
   * <p>Uses default SimpleCacheResolver which resolves cache by name.
   *
   ...
   *
   * <p>Generates cache keys in a consistent and readable format. Handles different parameter types
   * appropriately.
   */
  public static class CustomKeyGenerator implements KeyGenerator {

    private static final org.slf4j.Logger log =
        org.slf4j.LoggerFactory.getLogger(CustomKeyGenerator.class);
   ...
   *
   * <p>Handles cache errors gracefully by logging the error and allowing the application to
   * continue without cache. This prevents cache failures from breaking the application.
   */
  public static class CustomCacheErrorHandler extends SimpleCacheErrorHandler {
    ...
    public void handleCacheClearError(
        @NonNull RuntimeException exception, @NonNull org.springframework.cache.Cache cache) {
      log.error(
